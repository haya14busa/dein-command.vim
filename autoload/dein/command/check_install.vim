"=============================================================================
" FILE: autoload/dein/command/check_install.vim
" AUTHOR: haya14busa
" License: MIT license
"=============================================================================
...
function! dein#command#check_install#command(bang, range, args) abort
  let parser = s:parser()
  let options = parser.parse(a:bang, a:range, a:args)
  if empty(options)
    return
  endif
  return call('dein#check_install', has_key(options, 'plugin') ? [options.plugin] : [])
endfunction

function! dein#command#check_install#complete(arglead, cmdline, cursorpos) abort
